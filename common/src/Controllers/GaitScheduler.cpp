 ...
 */
template <typename T>
GaitScheduler<T>::GaitScheduler(KIST_UserParameters* _userParameters, float _dt) {
  initialize();
...
//   // Time based descriptors
//   periodTime = Vector4d::Zero();           // overall gait period time
//   timeStance = Vector4d::Zero();           // total stance time
//   timeSwing = Vector4d::Zero();            // total swing time
//   timeStanceRemaining = Vector4d::Zero();  // stance time remaining
//   timeSwingRemaining = Vector4d::Zero();   // swing time remaining

//   // Phase based descriptors
//   switchingPhase = Vector4d::Zero();  // phase to switch to swing
//   phaseVariable = Vector4d::Zero();   // overall gait phase for each foot
//   phaseOffset = Vector4d::Zero();     // nominal gait phase offsets
//   phaseScale = Vector4d::Zero();      // phase scale relative to variable
//   phaseStance = Vector4d::Zero();     // stance subphase
//   phaseSwing = Vector4d::Zero();      // swing subphase
...
//  * Constructor to automatically setup a basic gait
//  */
// GaitScheduler::GaitScheduler(KIST_UserParameters* _userParameters, float _dt) {
//   initialize();
//   userParameters = _userParameters;
//   dt = _dt;
// }
...
//     // Adjust the gait parameters
//     if (fabs(gaitData.periodTimeNominal - userParameters->gait_period_time) > 0.0001 ||
//         fabs(gaitData.switchingPhaseNominal - userParameters->gait_switching_phase) > 0.0001) {
//       // Modify the gait with new parameters if it is overrideable
//       if (gaitData.overrideable == 1) {
//         gaitData.periodTimeNominal = userParameters->gait_period_time;
//         gaitData.switchingPhaseNominal = userParameters->gait_switching_phase;
...
//     gaitData.gaitName = "TRANSITION_TO_STAND";
//     gaitData.gaitEnabled << 1, 1, 1, 1;
//     float oldGaitPeriodTimeNominal = gaitData.periodTimeNominal;
//     gaitData.periodTimeNominal = 3 * gaitData.periodTimeNominal;
//     gaitData.initialPhase = 0.0;
//     gaitData.switchingPhaseNominal =
//       (gaitData.periodTimeNominal + oldGaitPeriodTimeNominal
