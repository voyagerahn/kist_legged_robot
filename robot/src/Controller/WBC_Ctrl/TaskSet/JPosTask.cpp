...
JPosTask<T>::JPosTask(const FloatingBaseModel<T>* robot)
    : Task<T>(a1::num_act_joint), robot_sys_(robot) {
  TK::Jt_ = DMat<T>::Zero(a1::num_act_joint, a1::dim_config);
  (TK::Jt_.block(0, 6, a1::num_act_joint, a1::num_act_joint))
      .setIdentity();
  TK::JtDotQdot_ = DVec<T>::Zero(a1::num_act_joint);

  _Kp = DVec<T>::Constant(a1::num_act_joint, 50.);
  _Kd = DVec<T>::Constant(a1::num_act_joint, 5.);
}
...
bool JPosTask<T>::_UpdateCommand(const void* pos_des, const DVec<T>& vel_des,
  ...
  for (size_t i(0); i < a1::num_act_joint; ++i) {
