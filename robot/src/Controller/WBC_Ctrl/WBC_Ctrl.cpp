  ...
  _wbcLCM(getLcmUrl(255))
{
  ...
  _model = model;
  _kin_wbc = new KinWBC<T>(a1::dim_config);

  _wbic = new WBIC<T>(a1::dim_config, &(_contact_list), &(_task_list));
  ...
  _Kp_joint.resize(a1::num_leg_joint, 5.);
  _Kd_joint.resize(a1::num_leg_joint, 1.5);

  //_Kp_joint_swing.resize(cheetah::num_leg_joint, 10.);
  //_Kd_joint_swing.resize(cheetah::num_leg_joint, 1.5);

  _state.q = DVec<T>::Zero(a1::num_act_joint);
  _state.qd = DVec<T>::Zero(a1::num_act_joint);
}
...
void WBC_Ctrl<T>::_UpdateLegCMD(ControlFSMData<T> & data){
  ...
  for (size_t leg(0); leg < a1::num_leg; ++leg) {
    cmd[leg].zero();
    for (size_t jidx(0); jidx < a1::num_leg_joint; ++jidx) {
      cmd[leg].tauFeedForward[jidx] = _tau_ff[a1::num_leg_joint * leg + jidx];
      cmd[leg].qDes[jidx] = _des_jpos[a1::num_leg_joint * leg + jidx];
      cmd[leg].qdDes[jidx] = _des_jvel[a1::num_leg_joint * leg + jidx];
